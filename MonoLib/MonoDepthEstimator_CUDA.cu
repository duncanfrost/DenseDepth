...
__global__ void updatePhotoErrorBW(Matrix3f R, Vector3f T,
                                   Intrinsics K,
                                   Vector2i imgSize,
                                   float *photo_error,
                                   int *nUpdates,
                                   ...
                                   int offsetSize = 0)
{
    ...
    for (unsigned int z = 0; z < depthSamples; z++)
    {
        ...
        if (PointInImage(pointTrackImage, imgSize) && pointTrack[2] > 0)
        {
            ...
            float patchError = 0;
            int nPixels = 0;
            for (int xOffset = -offsetSize; xOffset <= offsetSize; xOffset++)
                for (int yOffset = -offsetSize; yOffset <= offsetSize; yOffset++)
                {

                    Vector2f pointTrackNew = pointTrackImage + Vector2f(xOffset,yOffset);
                    Vector2f refPointNew = Vector2f(x,y) + Vector2f(xOffset,yOffset);

                    if (!PointInImage(pointTrackNew, imgSize) || !PointInImage(refPointNew, imgSize))
                        continue;

                    //L1 norm of the photometric error
                    float photo_current_OR = interpolateBilinearUChar(currImageData,pointTrackImage,imgSize.x);
                    Vector4u photo_ref = refImageData[(int)refPointNew.x + imgSize.x*(int)refPointNew.y];
                    float normL1 = PhotoErrorL1BW(photo_current_OR,photo_ref);
                    patchError += normL1;
                    nPixels++;
                }
        ...
        {
            // float normL1=10000.0f;//normL1 in [0 1]
            // float oldError = photo_error[offset];
            // float obsError = normL1;
            // float newError = (nUpdate * oldError + obsError) / (nUpdate + 1);
        ...
        if (PointInImage(pointTrackImage, imgSize) && pointTrack[2] > 0)
        {
            ...
            for (int xOffset = -offsetSize; xOffset <= offsetSize; xOffset++)
                for (int yOffset = -offsetSize; yOffset <= offsetSize; yOffset++)
                {
                    ...
                    //L1 norm of the photometric error
                    Vector4f photo_current_OR = interpolateBilinearVec4(currImageData,pointTrackImage,imgSize.x);
                    Vector4u photo_ref = refImageData[(int)refPointNew.x + imgSize.x*(int)refPointNew.y];
                    float normL1 = PhotoErrorL1(photo_current_OR,photo_ref);
                    patchError += normL1;
                    nPixels++;
...
{
    std::cout << "Mono sampling here" << std::endl;
    float depthIncrement = (optimPyramid->maxIDepth - optimPyramid->minIDepth) /
        (float)optimPyramid->depthSamples;
    ...
    updatePhotoErrorBW<<<blocks2,threadsPerBlock2>>>(refToTracker.GetR(),
                                                     refToTracker.GetT(),
                                                     monoLevel->intrinsics,
                                                     imgSize,
                                                     optimPyramid->photoErrors->GetData(MEMORYDEVICE_CUDA),

...
Sophus::SE3f RMDEngine::invRefPose;

RMDEngine::RMDEngine(ImageSource* source, DepthSource* depthSource,
...
void RMDEngine::AddKeyFrame(ORUChar4TSImage *inImage, Sophus::SE3f inPose)
{
...
void RMDEngine::SmoothPhoto(int iterations)
{
...
void RMDEngine::SmoothPhotoBuffer(int iterations)
{
    SampleFromBufferMid();
...
void RMDEngine::MakePointCloud(bool useRawDepth)
{
...
void RMDEngine::GetPointCloud(unsigned int &width,
...
void RMDEngine::SampleFromBufferMid()
{
...
void RMDEngine::SaveToBuffer(ORUChar4TSImage *inputRGBImage,
...
void RMDEngine::WritePhotoErrors(std::string path)
{
    monoDepthEstimator->WritePhotoErrors(path);
}

cv::Mat RMDEngine::PreProcessImage(cv::Mat image)
{
    cv::Size outSize;
